return PlaceObj('ModDef', {
	...
	'last_changes', "5.2 fix changes Major Hotfix #3\n\nHotfix #3.1 Added save game cleanup code that will spawn the cheatdeleted unit from the below bug.\n\nHotfix #2.1: Made out of bounds checker code stricter to only check animals or robots; and explicitly went back to how the base game makes its determination (instead of trying to add another condition)\nThis fixes the bug where colonists will dissapear when going on an expedition or going close to the edge of the map.\n\nHotfix #1.1 Bugfix: Ulfen now will get constipation when they are tamed; instead of when they are wild and free.\n\n#1 New Option/mechanic: Pregnancy now has a 20% base chance of spawning a units evolved form.\nThis can be edited via a mod option, from 0 to 100.\nMeaning you can set the game to never give you evolved forms via pregnancy or set it (back) to a garuntee.\n\n#2 Bugfix (Technically for Nests Awaken, but the chance needs to be in this mod)\nAdded the UnitNesting component to all scissorhand evolutions\nShould also fix looping of spawns of high tier Scissorhands in the gmae\n\n#3 Bugfix Added code that runs everyday that:\n- Deletes all units outside of the map\n- Detects if there is a single species/evolution chain that has 500+ units on the map\n-- If there is 500+ it will delete all non-tamed animals in this chain to reset the game and save some frames\n\n#4 QoL debugged too hard, logs too noisy. Reduced debug logging for specific functions.\n\nPlease see the git in the mod description for open bugs, and even submit your own\nFeedback is always welcome to fine tune the balance of things!",
	'dependencies', {
